    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsComplex) {
                ...
                float orth_input_vals[kNItems];  // Store for gradient computation if using orth input
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                    float velocity_input;
                    if (params.use_orth_input) {
                        // Load from pre-computed orthogonalized tensor [B, D, L, N]
                        const int seq_idx = chunk * kChunkSize + threadIdx.x * kNItems + i;
                        if (seq_idx < params.seqlen) {
                            float *orth_ptr = reinterpret_cast<float *>(params.deltaB_u_orth_ptr);
                            velocity_input = orth_ptr[batch_id * params.deltaB_u_orth_batch_stride
                                                    + dim_id * params.deltaB_u_orth_d_stride
                                                    + seq_idx * params.deltaB_u_orth_seqlen_stride
                                                    + state_idx * params.deltaB_u_orth_dstate_stride];
                        } else {
                            velocity_input = 0.f;
                        }
                        orth_input_vals[i] = velocity_input;
                    } else {
                        float B_delta_u = !kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * float(u_vals[i]) * B_vals[i];
                        velocity_input = params.alpha * B_delta_u;
                        orth_input_vals[i] = 0.f;  // Not used
                    }
                    velocity_data[i] = make_float2(params.beta, velocity_input);
                }
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    if (params.use_orth_input) {
                        // When using orthogonalized input, store gradient to d_deltaB_u_orth
                        // dv is ∂L/∂orth_input (the gradient w.r.t. the orthogonalized velocity input)
                        const int seq_idx = chunk * kChunkSize + threadIdx.x * kNItems + i;
                        if (seq_idx < params.seqlen && params.d_deltaB_u_orth_ptr != nullptr) {
                            float *d_orth_ptr = reinterpret_cast<float *>(params.d_deltaB_u_orth_ptr);
                            gpuAtomicAdd(&d_orth_ptr[batch_id * params.d_deltaB_u_orth_batch_stride
                                                   + dim_id * params.d_deltaB_u_orth_d_stride
                                                   + seq_idx * params.d_deltaB_u_orth_seqlen_stride
                                                   + state_idx * params.d_deltaB_u_orth_dstate_stride], dv);
                        }
                        // Still compute gradient for exp term (affects ddelta through A path)
                ...
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                    weight_t velocity_input_val;
                    if (params.use_orth_input) {
                        // Load from pre-computed orthogonalized tensor [B, D, L, N] (complex)
                        const int seq_idx = chunk * kChunkSize + threadIdx.x * kNItems + i;
                        if (seq_idx < params.seqlen) {
                            float *orth_ptr = reinterpret_cast<float *>(params.deltaB_u_orth_ptr);
                            float real_val = orth_ptr[batch_id * params.deltaB_u_orth_batch_stride
                                                    + dim_id * params.deltaB_u_orth_d_stride
                                                    + seq_idx * params.deltaB_u_orth_seqlen_stride
                                                    + state_idx * params.deltaB_u_orth_dstate_stride * 2];
                            float imag_val = orth_ptr[batch_id * params.deltaB_u_orth_batch_stride
                                                    + dim_id * params.deltaB_u_orth_d_stride
                                                    + seq_idx * params.deltaB_u_orth_seqlen_stride
                                                    + state_idx * params.deltaB_u_orth_dstate_stride * 2 + 1];
                            velocity_input_val = complex_t(real_val, imag_val);
                        } else {
                            velocity_input_val = complex_t(0.f, 0.f);
                        }
                    } else {
                        weight_t B_delta_u_val = !kIsVariableB ? delta_vals[i] * float(u_vals[i]) : B_vals[i] * delta_vals[i] * float(u_vals[i]);
                        velocity_input_val = complex_t(params.alpha * B_delta_u_val.real_, 
                                                      params.alpha * B_delta_u_val.imag_);
                    }
                    velocity_data[i] = make_float4(params.beta, 0.f, 
                                                   velocity_input_val.real_, 
                                                   velocity_input_val.imag_);
                }
                // Load velocity running prefix from even indices
                scan_t v_running_prefix = chunk > 0 && threadIdx.x % 32 == 0 ? 
                    x[(chunk - 1) * params.dstate * 2 + state_idx * 2] : 
                    ...
                    if (params.use_orth_input) {
                        ...
                        if (seq_idx < params.seqlen && params.d_deltaB_u_orth_ptr != nullptr) {
                            ...
                            gpuAtomicAdd(&d_orth_ptr[batch_id * params.d_deltaB_u_orth_batch_stride
                                                   + dim_id * params.d_deltaB_u_orth_d_stride
                                                   + seq_idx * params.d_deltaB_u_orth_seqlen_stride
                                                   + state_idx * params.d_deltaB_u_orth_dstate_stride * 2], dv.real_);
                            gpuAtomicAdd(&d_orth_ptr[batch_id * params.d_deltaB_u_orth_batch_stride
                                                   + dim_id * params.d_deltaB_u_orth_d_stride
                                                   + seq_idx * params.d_deltaB_u_orth_seqlen_stride
                                                   + state_idx * params.d_deltaB_u_orth_dstate_stride * 2 + 1], dv.imag_);
                        }
                        // Still compute gradient for exp term
