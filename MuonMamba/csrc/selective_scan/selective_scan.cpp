...
void set_ssm_params_fwd(SSMParamsBase &params,
                        ...
                        void* deltaB_u_orth_ptr = nullptr,
                        const at::Tensor* deltaB_u_orth = nullptr) {
    ...
    // Set orthogonalized input pointer and strides (shape: [B, D, L, N])
    params.deltaB_u_orth_ptr = deltaB_u_orth_ptr;
    if (use_orth_input && deltaB_u_orth != nullptr) {
...
void set_ssm_params_bwd(SSMParamsBwd &params,
                        ...
                        void* deltaB_u_orth_ptr = nullptr,
                        const at::Tensor* deltaB_u_orth = nullptr,
                        void* d_deltaB_u_orth_ptr = nullptr,
                        const at::Tensor* d_deltaB_u_orth = nullptr) {
    ...
    set_ssm_params_fwd(params, batch, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       ...
                       use_orth_input, deltaB_u_orth_ptr, deltaB_u_orth);
    ...
    // Set gradient output for orthogonalized input (shape: [B, D, L, N])
    params.d_deltaB_u_orth_ptr = d_deltaB_u_orth_ptr;
    if (use_orth_input && d_deltaB_u_orth != nullptr) {
...
selective_scan_fwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    // Validate orthogonalized input if provided (shape: [B, D, L, N])
    at::Tensor deltaB_u_orth;
    if (use_orth_input) {
        deltaB_u_orth = deltaB_u_orth_.value();
        TORCH_CHECK(deltaB_u_orth.is_cuda());
        TORCH_CHECK(deltaB_u_orth.is_contiguous());
        CHECK_SHAPE(deltaB_u_orth, batch_size, dim, seqlen, dstate);
    }
    ...
    set_ssm_params_fwd(params, batch_size, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       ...
                       alpha,
                       use_orth_input,
                       use_orth_input ? deltaB_u_orth.data_ptr() : nullptr,
                       use_orth_input ? &deltaB_u_orth : nullptr);
...
selective_scan_bwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    if (use_orth_input) {
        ...
        TORCH_CHECK(deltaB_u_orth.is_contiguous());
        CHECK_SHAPE(deltaB_u_orth, batch_size, dim, seqlen, dstate);
        // Allocate gradient output tensor
        d_deltaB_u_orth = torch::zeros({batch_size, dim, seqlen, dstate}, deltaB_u_orth.options().dtype(torch::kFloat32));
    }
    ...
    set_ssm_params_bwd(params, batch_size, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       ...
                       use_orth_input ? deltaB_u_orth.data_ptr() : nullptr,
                       use_orth_input ? &deltaB_u_orth : nullptr,
                       use_orth_input ? d_deltaB_u_orth.data_ptr() : nullptr,
                       use_orth_input ? &d_deltaB_u_orth : nullptr);
    ...
    if (use_orth_input) { result.push_back(d_deltaB_u_orth); }
    return result;
}
