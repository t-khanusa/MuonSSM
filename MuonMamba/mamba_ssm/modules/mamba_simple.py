...
from mamba_ssm.ops.selective_scan_interface import selective_scan_fn, selective_scan_fn_orth, mamba_inner_fn
...
class Mamba(nn.Module):
    def __init__(
        ...
        device=None,
        dtype=None,
        beta=0.0,  # Momentum decay parameter
        alpha=1.0,  # Momentum scale parameter
        use_newton_schulz=False,  # Enable Newton-Schulz orthogonalization for Muon
        ...
        A = repeat(
            torch.arange(1, self.d_state + 1, **factory_kwargs),#dtype=torch.float32, device=device),
            "n -> d n",
            d=self.d_inner,
        ).contiguous()
        ...
        # D "skip" parameter
        self.D = nn.Parameter(torch.ones(self.d_inner, **factory_kwargs))  # Keep in fp32
        self.D._no_weight_decay = True

        # Momentum parameters (registered as buffers, not learnable)
        self.register_buffer("beta", torch.tensor(beta, **factory_kwargs))#dtype=torch.float32, device=device))
        self.register_buffer("alpha", torch.tensor(alpha, **factory_kwargs))#dtype=torch.float32, device=device))
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        # Note: Fast path doesn't support Newton-Schulz orthogonalization or outputting states
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None and not self.use_newton_schulz:
            out = mamba_inner_fn(
                xz,
                self.conv1d.weight,
                self.conv1d.bias,
            ...
            assert self.activation in ["silu", "swish"]
            # Choose scan function based on Newton-Schulz setting
            if self.use_newton_schulz:
                ...
                # Original Momentum without Newton-Schulz
                y = selective_scan_fn(
                    x,
                    dt,
                    A,
