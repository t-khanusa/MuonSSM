# Adapted from flash_muon: https://github.com/...
# Optimized X @ X.T computation exploiting symmetry
...
def mmt_kernel(
    ...
    """
    Core kernel that computes y = x @ x.T
    Exploits symmetry: only computes lower triangle, then mirrors.
    Adapted from triton matmul tutorial.
    """
    ...
    # Skip upper triangle (symmetry optimization)
    if pid_m > pid_n:
        return
    ...
    # a & b ptrs denote different rows of x
    ...
    # Copy to symmetric position (transpose)
    if pid_m < pid_n:
...
def matmul_transpose_assign(d_in, d_out):
    """
    Compute d_out = d_in @ d_in.T in-place.
    Optimized for symmetric output matrix.
    
    Args:
        d_in: Input tensor [M, K]
        d_out: Output tensor [M, M] (pre-allocated)
    """
    assert d_in.is_cuda, "Input must be CUDA tensor"
    assert d_out.is_cuda, "Output must be CUDA tensor"
    assert d_in.device == d_out.device, "Inputs must be on same device"
    assert d_in.dtype == d_out.dtype, "Inputs must have same dtype"
    assert d_in.ndim == 2, "Input must be 2D"
    assert d_out.ndim == 2, "Output must be 2D"
    assert d_in.size(0) == d_out.size(0) == d_out.size(1), \
        "First dim of d_in must match both dims of d_out"
...
def matmul_transpose(d_in):
    """
    Compute d_in @ d_in.T and return new tensor.
