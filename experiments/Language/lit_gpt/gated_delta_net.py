...
class GatedDeltaNet(nn.Module):
    def __init__(
        ...
        if use_mamba_gate:
            # Compute initialization in float32 for numerical stability, then cast to default dtype
            # This avoids precision issues while maintaining FSDP compatibility (uniform dtype)
            default_dtype = torch.get_default_dtype()
            A = torch.empty(self.num_heads, dtype=torch.float32).uniform_(0.01, 16)  # Avoid very small values
            A_log = torch.log(A).to(default_dtype)
            self.A_log = nn.Parameter(A_log)
            self.A_log._no_weight_decay = True
            self.D = nn.Parameter(torch.ones(self.num_heads, dtype=default_dtype))
            ...
            inv_dt = (dt + torch.log(-torch.expm1(-dt))).to(default_dtype)
    ...
    def forward(
        ...
        **kwargs
        ...
        q, conv_state_q = self.q_conv1d(q, attention_mask, conv_state_q)
        k, conv_state_k = self.k_conv1d(k, attention_mask, conv_state_k)
        v, conv_state_v = self.v_conv1d(v, attention_mask, conv_state_v)
