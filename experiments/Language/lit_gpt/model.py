...
# from .longhorn import MuonLonghorn, Longhorn
# from .muonmamba import MuonMamba, MuonMambaConfig
...
class GPT(nn.Module):
    def __init__(self, config: Config) -> None:
        super().__init__()
        # Use float32 initially - will be converted to bfloat16 by fabric.setup() for FSDP compatibility
...
class Block(nn.Module):
    def __init__(self, config: Config, layer_idx: int) -> None:
        ...
        # self.use_mamba = layer_idx % config.mamba_per_layer == 0 if config.mamba_per_layer > 0 else False
        ...
        if self.use_gated_deltanet:
            ...
            print(f"Using MuonLonghorn for layer {layer_idx}")
            ...
            # Use base Longhorn (without momentum/Newton-Schulz)
            print(f"Using Longhorn for layer {layer_idx}")
            # self.attn = Longhorn(
            #     d_model=config.n_embd,
            #     layer_idx=layer_idx,
            #     use_fast_path=False,
            self.attn = MuonLonghorn(
                d_model=config.n_embd,
                layer_idx=layer_idx,
                use_newton_schulz=False,  # Enable Newton-Schulz
                use_fast_path=False,
                beta=0.0,   # Momentum decay
                alpha=1.0,
            )
        # elif self.use_mamba:
    ...
    def forward(
        ...
        elif self.use_muon_longhorn or self.use_longhorn:
            h = self.attn(n_1)
            new_kv_cache = None
        # elif self.use_mamba:
        #     h = self.attn(n_1)
